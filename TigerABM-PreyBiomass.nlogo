...
; The meaning of all variables, their meaning, and their range of possible values are listed in Table 3.3 in the thesis

globals [all-female-centroids cub-females cub-males breeding-females breeding-males juvenile-females juvenile-males transient-males transient-females litter-size-list male-land-tenure-list fem-land-tenure-list prey-dataset borders-dataset churia-dataset wards-dataset min-prey max-prey too-far-female male-disp-dist-list fem-disp-dist-list num-infanticide total-cub-list cub-indep-list num-challenges dead-male-chall dead-fem-starv dead-cub-male dead-cub-fem dead-juv-male dead-juv-fem dead-tran-male dead-tran-fem dead-adult-fem dead-adult-male age-adult-male-died age-adult-fem-died prey-sum my-seed mean-male-territory-size mean-female-territory-size num-breeding-males num-breeding-females num-cub-males num-cub-females num-juv-males num-juv-females num-transient-males num-transient-females total-males total-females num-fem-w-off _recording-save-file-name]
...
[
  ;state variables:
;  is-churia? ; Indicates whether patch falls within churia hill boundary (Chitwan landscape only)
;  is-park? ; Indicates whether patch falls within national park boundary (Chitwan landscape only)
  ...
  dist-boundary ;this is used for scenario testing
]
...
to setup
  clear-all
  r:eval ".libPaths(c('C:/Users/snurf/AppData/Local/R/win-library/4.2',.libPaths()))" ; Change to the path on your device
  ...
  ;  ; We have to bring all the GIS data in and apply it the NetLogo landscape:
  ;  ; we first bring in the National Park polygon:
  ;  set borders-dataset gis:load-dataset "data_gis/CNP_45_UTM.shp"
  ;  ; Next we bring in a polygon for the Churia hill range inside the Park where tiger density is lower than the flatter areas of the Park:
  ;  set churia-dataset gis:load-dataset "data_gis/Churia_CNP.shp"
  ;  ; these are administrative boundaries outside the Park:
  ;  ; this is a raster GIS file (250m cell size) with prey production values:
  ;  set prey-dataset gis:load-dataset "data_gis/prey_prod_v6.asc"
  ;  ; this tells NetLogo how big the "world" should be, based on the intersection of the different GIS files:
  ;  gis:set-world-envelope ( gis:envelope-union-of ( gis:envelope-of borders-dataset ) ( gis:envelope-of prey-dataset )) ;(gis:envelope-of wards-dataset ))
  ;  ; this applies the prey value from the raster GIS file to each corresponding pixel (patch) of the NetLogo world:
  ;  gis:apply-raster prey-dataset prey
  ;  ; resets the patch coordinates based on the dimensions of the prey dataset:
  ;  resize-world 0 gis:width-of prey-dataset 0 gis:height-of prey-dataset
  ;  ; record the minimum prey production value from the prey dataset. This is used later to color the patches:
  ;  set min-prey gis:minimum-of prey-dataset
  ;  ; record the maximum prey production value from the prey dataset. This is used later to color the patches:
  ;  set max-prey gis:maximum-of prey-dataset
  ;  ; gis"apply-raster produces "NaN" for some unknown reason. The use of the "<= 0 or >= 0" technique filters out
  ;  ; "not a number" values
  ;  ; the NaN values are then set to "0"
  ;
  ;  ask patches
  ;  [
  ;    ifelse (prey <= 0) or (prey >= 0)
  ...
  ;    set is-churia? false
  ;  ]
  ;  ; we tell patches inside National Park polygon that they are "in-park"
  ;  ask patches gis:intersecting borders-dataset
  ;  [
  ;    set is-park? true
  ;  ]
  ;  ask patches with [is-park? = false] [ set pcolor white]
  ;  ; we tell patches inside Churia hills polygon that they are "in-churia"
  ;  ask patches gis:intersecting churia-dataset
  ;  [
  ;   set is-churia? true
  ;  ]
  ;  ; Females are created and distributed in different parts of the region
  ;  ; First, 80% of the initial number of females are distributed inside the park but not in the Churia hills, with a minimum distance between them of 12 patches:
  ;  create-females (0.80 * 28)
  ;  [
  ;    move-to one-of patches with [is-park? and prey > 0 and is-churia? = false and not any? other females in-radius 12]
  ...
  ;  ]
  ;  ; Now, distribute 20% of the initial number of females in the park and in the Churia hills:
  ;  create-females (0.22 * 28)
  ;  [
  ;    move-to one-of patches with [is-park? and prey > 0 and is-churia? and not any? other females in-radius 12]
  ;    set age 36 + random (180 - 84)
  ;    set age-years (floor (age / 12) + 1)
  ;    set age-class "breeder"
  ;    set shape "square"
  ...
  ;                               ; My-mom must be set to non-zero value for "male-select-location" to work properly
  ;  ]
  ;  ; Males are created and distributed in different parts of the region
  ;  ; First, 80% of the initial number of males are distributed inside the park but not in the Churia hills, with a minimum distance between them of 20 patches:
  ;  create-males (0.80 * 14)
  ;  [
  ;    move-to one-of patches with [is-park? and prey > 0 and is-churia? = false and not any? other males in-radius 20]
  ;    set age 36 + random (180 - 84)
  ;    set age-years (floor (age / 12) + 1)
  ;    set age-class "breeder"
  ;    set shape "triangle"
  ...
  ;                               ; My-mom must be set to non-zero value for "male-select-location" to work properly
  ;  ]
  ;  ; Now, distribute 20% of the initial number of males in the park and in the Churia hills:
  ;  create-males (0.22 * 14)
  ;  [
  ;    move-to one-of patches with [is-park? and prey > 0 and is-churia? and not any? other males in-radius 20]
  ;    set age 36 + random (180 - 84)
  ;    set age-years (floor (age / 12) + 1)
  ;    set age-class "breeder"
  ;    set shape "triangle"
  ...
  ; Reproducing the four prey biomass patterns described by Carter et al. (2015):
  ;   1. Homogenous. Prey biomass production of 5 kg
  ...
  ;   4. Left-right gradient going from lowest (2.05 kg) to highest (10.46) prey biomass production
  ...
  [
    ...
    ; Set the value of prey biomass production for all patches
    (
      ifelse
      prey-biomass = "homogenous"
      [
        set prey 5
      ]
  ...
  [
    move-to one-of patches with [prey > 0 and not any? other females in-radius 12]
    set age 36 + random (180 - 84)
    set age-years (floor (age / 12) + 1)
    set age-class "breeder"
    set terr-orig patch-here
...
;  if ticks = 680 [stop] ; Chitwan landscape only
...
;    set vacant (patch-set vacant neighbors4 with [owner-fem = nobody and is-park?]) ; R1 (Chitwan landscape only)
...
;    set owned (patch-set owned neighbors4 with [owner-fem != nobody and owner-fem != calling-fem and is-park?]) ; R1 (Chitwan landscape only)
...
to female-select-location
  ...
  let potential-sites in-radius-sites ; with [is-park?] (Chitwan landscape only)
...
to calc-homerange
  ...
  r:eval "write.csv(hr.sizes, file='C:/NetLogo/Tiger_ABM/hr_sizes.csv')" ; change the path to the preferred location on your device
...
Most items in the Interface tab were copied from Carter et al. (2015)'s model, except for:
  ...
  1. _num-of-fem_, which represents the number of adult females created at the beginning of a simulation,
2. _prey-biomass_, which represents the prey biomass pattern, and
3. _hide-breeders?_, which gives two options: to hide or to show the adult tigers.

Users can control the number of female tigers (using the slider: _num-of-fem_) and the spatial distribution of prey biomass production (using the chooser: _prey-biomass_).

The switch _hide-breeders?_ can also be turned on or off based on preference. The turtles representing the tigers will be hidden from view if it is turned on, and vice versa.
...
This model utilises the R extension of NetLogo to estimate the home range size with the "adehabitatHR" package from the R software.
...
<experiments>
  ...
  <experiment name="my-terr-growth" repetitions="1" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <final>export-view (word "C:/Users/snurf/Desktop/export/V tg" terr-growth " " prey-biomass "_" num-of-fem " tick" ticks ".png")</final>
    <timeLimit steps="12"/>
